...
void main() {
  ...
  test(
    'v3 -> v4 upgrade adds the wire-half columns, keeping existing rows (NULLs)',
    () async {
      ...
      await v3.customStatement(
        'ALTER TABLE messages DROP COLUMN signed_client_msg_id',
      );
      await v3.customStatement(
        'ALTER TABLE messages DROP COLUMN origin_crypto_valid',
      );
      await v3.customStatement(
        'ALTER TABLE channels DROP COLUMN ordinal',
      ); // v5 col
      ...
      expect(
        await rowCount(v4),
        1,
        reason: 'the v3 row must survive the migration',
      );
      final row = await singleMessage(v4);
      expect(row.read<String>('body'), 'survives migration');
      expect(
        row.readNullable<String>('signed_client_msg_id'),
        isNull,
        reason: 'a pre-v4 row has no signed client_msg_id',
      );
      expect(
        row.readNullable<int>('origin_crypto_valid'),
        isNull,
      ...
      final fresh = await v4
          .customSelect(
            "SELECT * FROM messages WHERE client_temp_id = 'row-v4'",
          )
          .getSingle();
  ...
  test(
    'v4 -> v5 upgrade adds channels.ordinal, keeping existing channel rows',
    () async {
  ...
  test(
    'v2 -> v4 upgrade adds the v3 signing columns AND the v4 wire columns',
    () async {
  ...
  test(
    'v5 -> v6 upgrade creates the retracted_ids table, keeping existing rows',
    () async {
      ...
      expect(
        (await singleMessage(v6)).read<String>('body'),
        'survives migration',
      );
  ...
  test(
    'v1 -> v4 upgrade recreates sync_meta AND adds every signing column',
    () async {
