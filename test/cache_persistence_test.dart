...
void main() {
  ...
  test(
    'messages and the B4 watermark survive a close + reopen of the same file',
    () async {
      ...
      expect(
        ...
        '01ULID_A',
        reason:
            'the reconnect-resume watermark must survive a restart — '
  ...
  test(
    'two per-user files are isolated — no cross-user history leak',
    () async {
      ...
      expect(
        ...
        isEmpty,
        reason:
            'a different user on the same device must never read another '
  ...
  group(
    'cacheFileName — injective + filesystem-safe (the isolation primitive)',
    () {
      ...
      test('output is always lowercase-hex and path-safe', () {
        for (final id in [
          ...
          expect(
            cacheFileName(id),
            matches(RegExp(r'^cache_[0-9a-f]*\.sqlite$')),
            reason:
                'a user id must never inject path separators or non-hex '
