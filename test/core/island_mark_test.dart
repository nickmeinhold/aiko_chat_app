...
void main() {
  ...
  group('the hash is STABLE', () {
    test(
      'pinned to fixed values — this is the whole promise, so it is nailed '
      ...
      () {
        ...
        expect(
          islandHash('chat.imagineering.cc'),
          islandHash('chat.imagineering.cc'),
        );
  ...
  group('the COMBINATION is the identity', () {
    test('colour and shape come from different parts of the hash, so two '
      ...
      final hosts = [for (var i = 0; i < 200; i++) 'island$i.example.org'];
      ...
      expect(
        ...
        isNot(b.shapeSeed),
        reason:
            'same colour AND same shape would make two islands '
            'indistinguishable',
      );
    });
    ...
    test('the colour space is CONTINUOUS, not a handful of buckets', () {
      ...
      expect(
        ...
        greaterThan(100),
        reason:
            'only ${colours.length} distinct colours across 200 islands '
  ...
  group('it is actually pressable', () {
    ...
    testWidgets('a tap in the CORNER of the target still counts — the dead '
      ...
      expect(
        ...
        1,
        reason:
            'a corner press missed — the hit test is still '
    ...
    testWidgets('with no onTap it stays a decoration and takes no space it '
      ...
      expect(
        tester.getSize(find.byType(IslandMark)).width,
        18,
  ...
  group('the KEY is the identity, the URL is a fallback', () {
    ...
    test('when the island publishes a key, the mark follows the KEY', () {
      ...
      final byKey = IslandIdentity.of('chat.example.org', islandPubkey: pubkey);
    ...
    test('and a DIFFERENT key at the SAME address is a DIFFERENT island — '
      ...
      final incumbent = IslandIdentity.of(
        'chat.example.org',
        islandPubkey: pubkey,
      );
      final squatter = IslandIdentity.of(
        'chat.example.org',
    ...
    test(
      'an empty key is not a key — fall back rather than derive from ""',
      () {
        expect(
          IslandIdentity.of('chat.example.org', islandPubkey: '').water,
  ...
  group('the SHAPES differ, not just the colours', () {
    ...
    test('islands come in DIFFERENT PROPORTIONS — some long, some fat', () {
      ...
      expect(
        ...
        lessThan(1.35),
        reason:
            'nothing is round — everything is a spit (roundest '
      ...
      expect(
        ...
        greaterThan(1.8),
        reason:
            'nothing is LONG — every island is the same egg (longest '
      ...
      expect(
        ...
        greaterThan(0.6),
        reason:
            'the range of proportions is too narrow to read as different '
            'places',
      );
    });
    ...
    test('two islands do not paint the same coastline', () {
      expect(
        elongation('chat.imagineering.cc'),
    ...
    test(
      'every island still FITS ITS DISC — a coastline that escapes the water '
      'is not an island',
      () {
        for (var i = 0; i < 60; i++) {
          ...
          expect(
            ...
            reason: 'island$i spills left',
          );
          expect(
            b.top,
            greaterThanOrEqualTo(2.0),
            reason: 'island$i spills up',
          );
          expect(
            b.right,
            lessThanOrEqualTo(98.0),
            reason: 'island$i spills right',
          );
          expect(
            b.bottom,
            lessThanOrEqualTo(98.0),
