...
void main() {
  group('classification is from CLOSED inputs only', () {
    test('transport failures map to their own transient reasons', () {
      expect(
        PushFailure.of(_dio(DioExceptionType.connectionError)),
        PushFailure.unreachable,
      );
      expect(
        PushFailure.of(_dio(DioExceptionType.connectionTimeout)),
        PushFailure.timedOut,
      );
      expect(
        PushFailure.of(_dio(DioExceptionType.sendTimeout)),
        PushFailure.timedOut,
      );
      expect(
        PushFailure.of(_dio(DioExceptionType.receiveTimeout)),
        PushFailure.timedOut,
      );
      expect(
        PushFailure.of(_dio(DioExceptionType.cancel)),
        PushFailure.cancelled,
      );
      expect(
        PushFailure.of(_dio(DioExceptionType.badCertificate)),
    ...
    test('a dead credential is distinguishable from every other 4xx', () {
      ...
      expect(
        PushFailure.forbidden.credentialIsDead,
        isFalse,
        reason:
            'a live credential refused an operation is not a dead one — '
            'claude-tasks#3723 branches on this',
      );
      // NOT credentialRejected — a 400 is our bug, not a dead session.
      expect(
        PushFailure.of(_dio(DioExceptionType.badResponse, status: 400)),
        PushFailure.rejected,
      );
      expect(
        PushFailure.of(_dio(DioExceptionType.badResponse, status: 404)),
        PushFailure.rejected,
      );
    });
    ...
    test('the RETRYABLE 4xx are not collapsed into permanent rejection', () {
      ...
      expect(
        PushFailure.of(_dio(DioExceptionType.badResponse, status: 429)),
        PushFailure.rateLimited,
      );
      expect(PushFailure.rateLimited.transient, isTrue);
      expect(
        PushFailure.of(_dio(DioExceptionType.badResponse, status: 408)),
        PushFailure.timedOut,
      );
    ...
    test('5xx is the island failing, and is worth retrying', () {
      expect(
        PushFailure.of(_dio(DioExceptionType.badResponse, status: 500)),
        PushFailure.islandError,
      );
      expect(
        PushFailure.of(_dio(DioExceptionType.badResponse, status: 503)),
        PushFailure.islandError,
      );
  ...
  group('the facts the enum carries', () {
    test('exactly one reason means the credential is dead', () {
      final lost = PushFailure.values.where((f) => f.credentialIsDead).toList();
      expect(
        lost,
        [PushFailure.credentialRejected],
        reason:
            'credentialIsDead is the flag that answers "can a retry ever '
      ...
      for (final f in PushFailure.values) {
        if (f.credentialIsDead) {
          expect(
            ...
            isFalse,
            reason:
                '${f.name} claims both a dead credential and a '
  ...
  group('the reason cannot carry a secret BY CONSTRUCTION', () {
    ...
    test(
      'POSITIVE CONTROL: the secret really is in the exception we passed',
      () {
        ...
        expect(
          e.toString().contains(_secret),
          isTrue,
          reason:
              'the fixture must actually carry the secret, or the '
    ...
    test('RAW fields carry no payload — asserted BEFORE redaction', () {
      ...
      PushTelemetry(
        log.child('push'),
      ).unregisterDeferred(_dio(DioExceptionType.badResponse, status: 401));
      ...
      expect(
        r.fields['credentialIsDead'],
        isTrue,
        reason:
            'both facts the type carries must reach the record — a '
      ...
      for (final e in r.fields.entries) {
        expect(
          e.value.toString().contains(_secret),
          isFalse,
