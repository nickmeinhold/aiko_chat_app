...
void main() {
  ...
  Future<FakeRestApi> openEditProfile(WidgetTester tester,
      {Object? updateThrows}) async {
  ...
  testWidgets('editing display name saves and updates the user', (tester) async {
  ...
  testWidgets('a taken handle shows an inline error and stays put',
    ...
    await tester.enterText(find.byType(TextField).at(0), 'taken_name'); // handle
    await tester.tap(find.widgetWithText(FilledButton, 'Save'));
    await tester.runAsync(
        () => Future<void>.delayed(const Duration(milliseconds: 50)));
    await tester.pumpAndSettle();
