...
void main() {
  ...
  testWidgets('logged out → login screen shows passkey sign-in', (
    tester,
  ) async {
  ...
  testWidgets(
    'passkey sign-in with an existing credential → straight to chat',
    (tester) async {
  ...
  testWidgets('claim-handle surfaces a taken handle inline', (tester) async {
    final rest =
        FakeRestApi() // default register outcome = PendingHandle
    ...
    expect(
      find.textContaining('That handle is taken — try another.'),
      findsOneWidget,
    );
    expect(find.widgetWithText(AppBar, 'Pick your handle'), findsOneWidget);
  });
  ...
  testWidgets('cold start with stored session → restores to chat', (
    tester,
  ) async {
  ...
  testWidgets(
    'short Terms that fit the screen enable Accept without scrolling',
    (tester) async {
      final container = makeContainer(
        rest: FakeRestApi(),
        transport: FakeChatTransport(),
