...
void main() {
  ...
  Future<MessageRow> rawRow() async => MessageRow.fromRow(
    await cache
        .customSelect(
  ...
  test('carriage LIVE: a signed send EMITS a valid, verifying origin on the wire '
  ...
  test(
    'the emitted SendFrame serialises origin as exactly the 7-key wire object',
    () async {
      final repo = buildRepo(signingKey: key);
      await repo.sendMessage(_chan, 'hi');
      final sent = transport.sent.single;
      ...
      expect((json['origin'] as Map).keys.toSet(), {
        'v',
        'alg',
        'key_version',
        'sender_pubkey',
  ...
  test(
    'unsigned send emits NO origin on the wire (legal absent state)',
    () async {
  ...
  test(
    'no signing key → no signature, no crash (graceful optionality)',
    () async {
      final repo = buildRepo(signingKey: null);
      await repo.sendMessage(_chan, 'hi');
      final row = await rawRow();
  ...
  test(
    'cage-match Carnot: a collapse (server body wins) CLEARS the stale sig',
    () async {
      final repo = buildRepo(signingKey: key);
      ...
      expect(
        after.body,
        'server body',
        reason: 'collapse merged server content',
      );
  ...
  test(
    'cage-match Tesla R2: a content-identical re-echo PRESERVES the sig',
    () async {
      final repo = buildRepo(signingKey: key);
      ...
      expect(
        after.sig,
        isNotNull,
        reason: 'unchanged content keeps the valid sig',
      );
  ...
  test(
    'cage-match Tesla R2: an inbound update with CHANGED body clears the sig',
    () async {
      final repo = buildRepo(signingKey: key);
  ...
  test(
    'cage-match Tesla R3: a content-identical COLLAPSE preserves the seal',
    () async {
      final repo = buildRepo(signingKey: key);
