...
void main() {
  ...
  testWidgets('llm sender shows an "AI" badge', (tester) async {
    await _pump(tester, const MessageSender(kind: SenderKind.llm, label: 'Aiko'));
    expect(find.byIcon(Icons.smart_toy), findsOneWidget);
    expect(find.text('AI'), findsOneWidget);
  });

  testWidgets('robot sender shows a "Robot" badge', (tester) async {
    await _pump(tester, const MessageSender(kind: SenderKind.robot, label: 'R2'));
    expect(find.byIcon(Icons.smart_toy), findsOneWidget);
  ...
  testWidgets('unknown island sender_kind (→ actor) still badges as "Bot"',
    ...
    final sender = MessageSender(kind: SenderKind.fromWire('agent'), label: 'Ag');
