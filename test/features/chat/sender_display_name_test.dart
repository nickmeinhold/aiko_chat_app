...
void main() {
  group('senderDisplayName — render current handle, not the send-time label', () {
    test('my own message uses my CURRENT handle, overriding a stale label', () {
    ...
    test('another member resolves to the roster CURRENT handle, not the label', () {
      final name = senderDisplayName(
    ...
    test('mine but handle not yet known falls through to roster/label', () {
      expect(
        senderDisplayName(_sender(userId: 'me', label: 'lbl'),
            isMine: true, myHandle: null, roster: const {'me': 'roster_me'}),
        'roster_me',
      );
      expect(
        senderDisplayName(_sender(userId: 'me', label: 'lbl'),
            isMine: true, myHandle: ''),
        'lbl',
      );
    });
