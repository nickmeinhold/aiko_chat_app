...
final Uint8List _goldenPub = Uint8List.fromList(
...
void main() {
  ...
  group('Multikey codec (mirrors the gateway decode_multikey)', () {
    ...
    test(
      'golden: the canonical Multikey matches the gateway byte-for-byte',
      () {
        expect(
          encodeMultikey(_goldenPub),
  ...
  group('validateOrigin — the single admission gate', () {
    test(
      'absent origin (null) returns null, never throws (unsigned is legal)',
      () {
    ...
    test('carriage is verifier-sufficient end-to-end', () async {
      ...
      final wire = OriginEnvelope.fromSignature(
        signature,
        clientMsgId: payload.clientMsgId,
      ).toWire();
      final rehydrated = jsonDecode(jsonEncode(wire)) as Map<String, dynamic>;
      final admitted = validateOrigin(
        rehydrated,
        frameClientMsgId: payload.clientMsgId,
      )!;
    ...
    group('RED — fail-closed rejections', () {
      void expectReject(Map<String, dynamic> Function() mutate) {
        expect(
          () => validateOrigin(mutate(), frameClientMsgId: _goldenCmid),
      ...
      test('not a JSON object', () {
        expect(
          () => validateOrigin('nope', frameClientMsgId: _goldenCmid),
          throwsA(isA<OriginError>()),
        );
      });
      test(
        'a missing key',
        () => expectReject(() => _validWire()..remove('sig')),
      );
      test(
        'an extra key',
        () => expectReject(() => _validWire()..['extra'] = 1),
      );
      test(
        'a wrong alg (alg-confusion)',
        () => expectReject(() => _validWire()..['alg'] = 'RS256'),
      );
      test(
        'an unsupported version',
        () => expectReject(() => _validWire()..['v'] = 2),
      );
      test(
        'v as a bool (JSON true must not satisfy == 1)',
        () => expectReject(() => _validWire()..['v'] = true),
      );
      test(
        'key_version < 1',
        () => expectReject(() => _validWire()..['key_version'] = 0),
      );
      test(
        'sender_pubkey not a Multikey',
        () => expectReject(
          () => _validWire()..['sender_pubkey'] = 'not-a-multikey',
        ),
      );
      ...
      test(
        'sig padded / non-canonical base64url',
        () => expectReject(() => _validWire()..['sig'] = '${'A' * 86}=='),
      );
      test('sig decodes to the wrong length', () {
        expectReject(
          () => _validWire()..['sig'] = base64UrlUnpadded(Uint8List(32)),
        );
      });
      test(
        'signed_at_ms negative',
        () => expectReject(() => _validWire()..['signed_at_ms'] = -1),
      );
      test(
        'client_msg_id mismatch with the frame (envelope-vs-payload confusion)',
        () {
          expect(
            () => validateOrigin(
              _validWire(),
              frameClientMsgId: 'a-different-id',
            ),
            throwsA(isA<OriginError>()),
          );
        },
