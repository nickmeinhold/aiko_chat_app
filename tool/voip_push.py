...
def cmd_send(args) -> int:
    host = HOSTS[args.env]
    # `--payload` sends the ISLAND'S REAL SHAPE — `{"aps":…, "c":…, "k":…}` —
    # rather than the spike harness's `mode`/`tag`. That distinction is the whole
    # value: `mode` exercises a throwaway arm selector inside `VoipSpike`, while
    # `--payload` exercises the PRODUCTION `CallKitRinger.handle` total function
    # against the exact keys the gateway emits. Same sender, two different
    # things under test, and conflating them would mean "the ring works" was
    # only ever proven about code that does not ship.
    if args.payload:
    ...
    what = args.payload if args.payload else f"mode={args.mode}"
...
def main() -> int:
    ...
    send.add_argument("--mode", default="report",
                      help="arm selector read by the SPIKE harness (VoipSpike)")
    send.add_argument("--payload",
                      help='raw JSON payload, e.g. \'{"c":"<channel>","k":"call_end"}\' — '
                           "exercises the PRODUCTION CallKitRinger, not the spike")
