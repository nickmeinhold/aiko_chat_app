# RFC-0005: The identity graph

| | |
|---|---|
| **RFC** | 0005 |
| **Status** | Draft, requesting comments |
| **Owner** | Nick Meinhold, with Claude (vocabulary foundation: RFC-0001) |
| **Created** | 2026-07-17 |
...
2. **Five edges.** `is-credential-of` (Self → Principal), `speaks-as` (Participant → Principal), `vouches-for` (Principal → Principal; bonded, conserved, slashable, per RFC-0006), `owns/runs` (Principal → Service), `is-instance-at` (→ ServiceTopicPath).
...
- **Why not a fourth identity concept for services?** The `chat.py` TODO hinted at one ("Owner"); reading the code showed Owner is already an edge (a pointer from a running Service to its accountable party), so promoting it to a node would mint exactly the duplicate-concept problem RFC-0001 exists to prevent.
